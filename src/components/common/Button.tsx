import { type ButtonHTMLAttributes, type FC, type ReactNode } from "react";
import { twMerge } from "tailwind-merge";

type ButtonProps = ButtonHTMLAttributes<HTMLButtonElement> & {
  children: ReactNode;
  color?: 'transparent' | 'white' | 'dark' | 'green';
  className?: string;
  onClick?: () => void;
  disabled?: boolean;
  icon?: ReactNode;
};

const colors = {
  transparent: "hover:bg-black/10 text-white disabled:text-gray-500",
  white: "bg-white/70 hover:bg-black/10 active:bg-white/70 text-gray-600 disabled:bg-black/30",
  dark: "bg-slate-900 hover:bg-slate-700 text-white disabled:bg-slate-500",
  green: "bg-green-medium text-white hover:bg-green-dark disabled:bg-green-medium/40"
};

const Button: FC<ButtonProps> = (props) => {
  const {
    children,
    color = 'white',
    className,
    onClick,
    disabled,
    icon,
    ...rest
  } = props;

  return (
    <button
      className={twMerge(
        "px-4 py-2 transition-all rounded-2xl active:scale-105 font-bold cursor-pointer disabled:cursor-not-allowed",
        colors[color],
        className,
      )}
      onClick={onClick}
      disabled={disabled}
      {...rest}
    >
      <div className="flex gap-2 items-center justify-center">
        {icon}
        {children}
      </div>
    </button>
  );
};

export default Button;
