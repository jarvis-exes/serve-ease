...
const ItemCard: FC<ItemsCardProps> = ({ item, setCart }) => {
    ...
    return <Card className='h-fit flex flex-col gap-2'>
        ...
        <div className='flex w-full h-24 gap-2 text-white cursor-pointer'>
            {!!prices.FULL &&
                <div className='w-1/2 flex flex-col justify-center items-center gap-2 rounded-xl p-2 bg-green-light active:scale-105'
            ...
            <div className='w-1/2 h-full flex flex-col gap-2'>
                {!!prices.HALF &&
                    <div className={twMerge('w-full h-full flex justify-center items-center gap-2 rounded-xl p-2 bg-green-medium active:scale-105',
                        !prices.QUARTER ? 'flex-col' : ''
                    )}>
                        {!!prices.QUARTER ? <span>H</span> : <span>Half</span>}
                        {prices.HALF}
                    </div>}

                {!!prices.QUARTER &&
                    <div className='w-full h-full flex justify-center items-center gap-2 rounded-xl p-2 bg-green-dark active:scale-105'>
                        <span>Q</span>
                        {prices.QUARTER}
                    </div>}
            </div>
