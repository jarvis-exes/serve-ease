...
function KitchenPage() {
    ...
    return (
        orders.length < 1 ?
            ...
            : <div className='bg-gray-200 h-full p-2 flex'>
                        ...
                        {orders?.filter(order => order.status !== 'READY').map(order =>
                            <Card className='flex flex-col w-full h-fit p-0'>
                                ...
                                <div className='p-2 flex justify-center'>
                                    <Button color='green' className='w-full rounded-xl' disabled={!isKitchen}
                                        onClick={() => handleOrderReady(order._id)}
                                    >
                                        {isKitchen ? 'Ready' : 'Preparing'}
                                    </Button>
                                </div>
                            </Card>
                        )}
                    ...
                    <div className='max-h-full overflow-auto p-2 pt-0 pb-9 gap-2 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3'>
                        {orders?.filter(order => order.status === 'READY').map(order =>
                            <Card className='flex flex-col w-full h-fit p-0'>
                                ...
                                <div className='p-2 flex justify-center'>
                                    <Button color='green' className='w-full rounded-xl bg-green-dark' disabled={isKitchen}
                                        onClick={() => handleOrderComplete(order._id)}
                                    >
                                        {isKitchen ? 'Prepared' : 'Complete'}
                                    </Button>
                                </div>
                            </Card>
                        )}
